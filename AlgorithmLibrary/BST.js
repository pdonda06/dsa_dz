...
BST.prototype.addControls = function() {
    ...
    this.insertButton = addControlToAlgorithmBar("Button", "Insert");
    this.insertButton.onclick = () => {
        const key = this.insertField.value;
        if (key) {
            this.insertCallback();
            this.operationHistory.push({ operation: 'insert', key });  // Track the operation
            updateOperationCode('insert', key);  // Dynamically update the code for Insert
        }
    };
    ...
    this.deleteButton.onclick = () => {
        const key = this.deleteField.value;
        if (key) {
            this.deleteCallback();
            this.operationHistory.push({ operation: 'delete', key });  // Track the operation
            updateOperationCode('delete', key);  // Dynamically update the code for Delete
        }
    };
    ...
    this.findButton.onclick = () => {
        const key = this.findField.value;
        if (key) {
            this.findCallback();
            this.operationHistory.push({ operation: 'search', key });  // Track the operation
            updateOperationCode('search', key);  // Dynamically update the code for Search
        }
    };
    ...
    // Add the Finish BST button
    this.finishButton = addControlToAlgorithmBar("Button", "Finish BST");
    this.finishButton.onclick = this.finishBST.bind(this);

    // Initialize the history of operations
    this.operationHistory = [];
};


// Insert callback - Records the operation
BST.prototype.insertCallback = function() {
    const value = this.insertField.value;
    if (value !== "") {
        this.operationHistory.push({ operation: 'insert', key: value });
        this.insertField.value = ""; // clear field
    }
};

// Delete callback - Records the operation
BST.prototype.deleteCallback = function() {
    const value = this.deleteField.value;
    if (value !== "") {
        this.operationHistory.push({ operation: 'delete', key: value });
        this.deleteField.value = ""; // clear field
    }
};

// Search callback - Records the operation
BST.prototype.findCallback = function() {
    const value = this.findField.value;
    if (value !== "") {
        this.operationHistory.push({ operation: 'search', key: value });
        this.findField.value = ""; // clear field
    }
};

// Finish BST - Generate code based on operations and language
BST.prototype.finishBST = function() {
    const selectedLanguage = document.getElementById("languageSelect").value;
    const codeDisplay = document.getElementById("codeDisplay");

    let finalCode = '';
    this.operationHistory.forEach((operation) => {
        const { operation: op, key } = operation;
        finalCode += this.generateCodeForOperation(op, key, selectedLanguage);
    });

    codeDisplay.innerHTML = `<pre>${finalCode}</pre>`;
};

// Generate code based on operation and selected language
BST.prototype.generateCodeForOperation = function(operation, key, language) {
    let code = '';

    if (operation === 'insert') {
        if (language === 'js') {
            code += `// JavaScript Insert for key ${key}\nfunction insert(node, key) {\n\tif (!node) return new Node(key);\n\tif (key < node.key) node.left = insert(node.left, ${key});\n\telse node.right = insert(node.right, ${key});\n\treturn node;\n}\n\n`;
        } else if (language === 'c') {
            code += `// C Insert for key ${key}\nNode* insert(Node* node, int key) {\n\tif (node == NULL) return newNode(${key});\n\tif (key < node->key) node->left = insert(node->left, ${key});\n\telse node->right = insert(node->right, ${key});\n\treturn node;\n}\n\n`;
        } else if (language === 'cpp') {
            code += `// C++ Insert for key ${key}\nNode* insert(Node* node, int key) {\n\tif (node == NULL) return new Node(${key});\n\tif (key < node->key) node->left = insert(node->left, ${key});\n\telse node->right = insert(node->right, ${key});\n\treturn node;\n}\n\n`;
        } else if (language === 'java') {
            code += `// Java Insert for key ${key}\nNode insert(Node node, int key) {\n\tif (node == null) return new Node(${key});\n\tif (key < node.key) node.left = insert(node.left, ${key});\n\telse node.right = insert(node.right, ${key});\n\treturn node;\n}\n\n`;
        } else if (language === 'python') {
            code += `# Python Insert for key ${key}\ndef insert(node, key):\n\tif not node:\n\t\treturn Node(${key})\n\tif key < node.key:\n\t\tnode.left = insert(node.left, ${key})\n\telse:\n\t\tnode.right = insert(node.right, ${key})\n\treturn node\n\n`;
        }
    }

    // Similarly generate for 'delete' and 'search' based on the language.
    
    return code;
};

// Clear everything and reset
BST.prototype.clearAll = function() {
    this.operationHistory = []; // Clear operation history
    const codeDisplay = document.getElementById("codeDisplay");
    codeDisplay.innerHTML = ''; // Clear displayed code
};
...
// Function to update the operation code displayed in the dropdown based on user's input
function updateOperationCode(operation, key) {
...
function search(node, key) {
    ...
    // Update the inner text of the operationCode element with the generated code
    operationCodeElement.textContent = generatedCode;
}

// Example: Call this function when an insert operation is performed with a user input
document.getElementById('insertButton').addEventListener('click', function() {
    const key = getUserInputValue(); // Replace this with actual logic to get the input value
    updateOperationCode('Insert', key);
});

// Similarly, for delete and search
document.getElementById('deleteButton').addEventListener('click', function() {
    const key = getUserInputValue(); // Replace this with actual logic to get the input value
    updateOperationCode('delete', key);
});

document.getElementById('searchButton').addEventListener('click', function() {
    const key = getUserInputValue(); // Replace this with actual logic to get the input value
    updateOperationCode('search', key);
});
